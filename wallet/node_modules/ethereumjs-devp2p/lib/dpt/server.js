...
const keccak256 = _require2.keccak256,
      pk2id = _require2.pk2id,
      createDeferred = _require2.createDeferred;
...
class Server extends EventEmitter {
  ...
  ping(peer) {
    ...
    return (0, _asyncToGenerator3.default)(function* () {
      ...
      const promise = _this._requestsCache.get(rckey);
      if (promise !== undefined) return promise;

      const hash = _this._send(peer, 'ping', {
        version: VERSION,
        from: _this._endpoint,
        to: peer
      });

      const deferred = createDeferred();
      const rkey = hash.toString('hex');
      _this._requests.set(rkey, {
        peer,
        deferred,
      ...
      _this._requestsCache.set(rckey, deferred.promise);
      return deferred.promise;
    })();
  }
