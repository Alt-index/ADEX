...
const int2buffer = _require2.int2buffer,
      buffer2int = _require2.buffer2int,
      assertEq = _require2.assertEq;
...
class ETH extends EventEmitter {
  ...
  sendStatus(status) {
    ...
    this._status = [int2buffer(this._version), int2buffer(status.networkId), status.td, status.bestHash, status.genesisHash];
